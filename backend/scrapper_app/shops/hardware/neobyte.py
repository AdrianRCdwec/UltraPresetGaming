    ...
    def escanear_catalogo(self, url_catalogo_base, categoria_db, tipo_db, excluir_palabras=None):
        ...
        try:
            while hay_mas_paginas:
                ...
                try:
                    self.page.wait_for_selector('article.product-miniature', state='attached', timeout=5000)
                except:
                    pass
                ...
                if pagina_actual == 1:
                    try:
                        btn_cookies = self.page.locator('.cookiesplus-reject').first
                        if btn_cookies.is_visible(timeout=3000):
                            btn_cookies.click()
                            logger.info("    🍪 Cookies rechazadas en NeoByte.")
