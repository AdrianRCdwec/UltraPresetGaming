...
class LifeInformaticaScraper(BaseScraper):
    ...
    def escanear_catalogo(self, url_catalogo_base, categoria_db, tipo_db, excluir_palabras=None):
        ...
        for intento in range(3):
            try:
                self.page.goto(url_catalogo_base, timeout=40000, wait_until="domcontentloaded")
        ...
        try:
            ...
            try:
                self.page.wait_for_selector('li.product.type-product', state='attached', timeout=5000)
            except:
                pass

            # Rechazar cookies (vital para que no tape el botón Cargar Más)
            try:
                btn_cookies = self.page.locator('#cf_consent-buttons__reject-all').first
                ...
                try:
                    ...
                    if boton_cargar_mas.is_visible(timeout=2000):
                        ...
                        if box:
                            ...
                            self.page.mouse.move(centro_x, centro_y, steps=random.randint(8, 15))
                            
                            # Pausa de duda antes de hacer clic
                            self.page.wait_for_timeout(random.uniform(100, 350))
                            self.page.mouse.click(centro_x, centro_y)
                        else:
