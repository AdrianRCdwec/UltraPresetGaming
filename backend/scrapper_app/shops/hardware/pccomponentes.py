    ...
    def escanear_catalogo(self, url_catalogo_base, categoria_db, tipo_db, excluir_palabras=None):
        ...
        try:
            while hay_mas_paginas:
                ...
                try:
                    self.page.wait_for_selector('a[data-testid="normal-link"]', state='attached', timeout=5000)
                except:
                    pass

                if pagina_actual == 1:
                    try:
                        btn_cookies = self.page.locator('#cookiesrejectAll').first
...
# Registramos el scraper en la Fábrica
ScraperFactory.registrar_scraper("pccomponentes", PcComponentesScraper)